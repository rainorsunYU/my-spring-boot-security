    ...
    var tpl = [
      ...
      <li>\
        {{# if(d.rows[i].collection_time){ }}\
          <a class="info-title" href="/info/{{d.rows[i].id}}/" target="_blank">{{= d.rows[i].title}}</a>\
        ...
        {{# } else { }}\
          ...
          {{# if(d.rows[i].accept >= 0){ }}\
            <span class="info-status info-status-ok">已结</span>\
          {{# } else { }}\
            <span class="info-status">未结</span>\
          {{# } }}\
          {{# if(d.rows[i].status == -1){ }}\
            <span class="info-status">审核中</span>\
          {{# } }}\
          <a class="info-title" href="/info/{{d.rows[i].id}}/" target="_blank">{{= d.rows[i].title}}</a>\
          <i class="layui-hide-xs">{{ layui.util.timeAgo(d.rows[i].time, 1) }}</i>\
          {{# if(d.rows[i].accept == -1){ }}\
          <a class="mine-edit layui-hide-xs" href="/info/edit/{{d.rows[i].id}}" target="_blank">编辑</a>\
          {{# } }}\
      ...
      if(gather.minelog[type + '-page-' + curr]){
        ...
        if(type === 'collection'){
          ...
          fly.json('/api/'+ type +'/', {
            ...
            view(res);
            gather.minelog['mine-info-page-' + curr] = res;
            now || laypage.render({
              elem: 'LAY_page'
              ,count: res.count
              ,curr: curr
